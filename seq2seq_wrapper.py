...
class Seq2Seq(object):
    ...
    def build_graph(self):
        ...
        basic_cell = DropoutWrapper(
            BasicLSTMCell(self.emb_dim, state_is_tuple=True),
            output_keep_prob=self.keep_prob)
        # stack cells together : n layered model
        stacked_lstm = MultiRNNCell([basic_cell] * self.num_layers, state_is_tuple=True)
        ...
        with tf.variable_scope('decoder') as scope:
            ...
            self.decode_outputs, self.decode_states = embedding_rnn_seq2seq(self.enc_ip, self.dec_ip, stacked_lstm,
                                                                            self.xvocab_size, self.yvocab_size,
                                                                            self.emb_dim)
            # share parameters
            ...
            self.decode_outputs_test, self.decode_states_test = embedding_rnn_seq2seq(
                self.enc_ip, self.dec_ip, stacked_lstm, self.xvocab_size, self.yvocab_size, self.emb_dim,
                feed_previous=True)
        ...
        self.loss = sequence_loss(self.decode_outputs, self.labels, loss_weights, self.yvocab_size)
